...
1. **Write it plainly.** Implement the task the ordinary way and run it until the output is right. No form pressure yet.
   **Output contract:** print exactly the number the example prints (the sum) and nothing else — do not add a mean, average, count, or range; the output check compares the exact number set on every input.

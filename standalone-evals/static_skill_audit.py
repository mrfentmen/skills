#!/usr/bin/env python3
"""
Static audit of every skill's SKILL.md. Scores 9 objective dimensions so we
...
Usage:
  python3 standalone-evals/static_skill_audit.py [--json]
  python3 standalone-evals/static_skill_audit.py --min-score 0.75   # exit 1 below
  (--min-score can be combined with --json)

This copy is vendored from the eval harness so the GitHub Actions gate runs
...
#   monorepo:       <root>/standalone-evals/static_skill_audit.py -> parent
...
THEME_KEYWORDS = {
    "god": ["creator", "architect", "invariant", "inspect", "design", "verify", "creation", "whole system"],
    ...
    "kyoka": ["kyoka", "comic", "punchline", "joke", "satir", "deflation"],
    "somonka": ["somonka", "reply", "exchange", "paired", "mirror", "answer"],
    "bussokusekika": ["bussokusekika", "seal", "verdict", "six lines", "5-7-5-7-7-7", "settle"],
    "imayo": ["imayo", "7-5", "pulse", "refrain", "song", "rolling"],
    "kanshi": ["kanshi", "couplet", "parallel", "mirror", "resolve", "turn"],
    "zappai": ["zappai", "moment", "three lines", "snapshot", "observation", "free"],
    "waka": ["waka", "turn", "resolve", "scene", "courtly", "5-7-5-7-7"],
    "renshi": ["renshi", "chain", "stages", "linked", "relay", "baton"],
    "sonnet": ["sonnet", "quatrain", "couplet", "volta", "fourteen", "turn"],
...
def main() -> None:
    ...
    parser.add_argument("--root", type=Path, default=BASE,
                        help="skill root to audit (default: script layout root)")
    args = parser.parse_args()
    root = args.root.resolve()
    single = (root / "SKILL.md").exists()
    if single:
    ...
    results = [audit_skill(s, root / "SKILL.md" if single
                           else root / s / "SKILL.md") for s in skills]
    for r in results:
    ...
    (root / "SKILL_AUDIT.json").write_text(
    ...
    if args.min_score is not None:
        worst = min((r["score"] for r in results), default=0.0)
        if worst < args.min_score:
            worst_name = min(results, key=lambda r: r["score"])["skill"] if results else "(none)"
            print(f"\nGATE FAILED: {worst_name} scored {worst:.2f} < "
                  f"required {args.min_score:.2f}, not shipping", file=sys.stderr)
            sys.exit(1)
        print(f"GATE PASSED: all skills >= {args.min_score:.2f}", file=sys.stderr)

...
Do not invent a renga from a blank page. Start by copying the first passing Python example in this skill, then adapt its slots to the user's task:
...
3. **Label the stanzas.** Copy the example's full-line `#` stanza labels (`# opening: ...`, `# pivot: ...`, `# closing: ...`) — a full-line comment is free (it never counts toward stanza size) and it makes the 3-2-3 alternation visible, so you cannot silently collapse a stanza. The closing stanza needs THREE real statements; if the task yields only two numbers, restate the result as a third print exactly as the example's `print("total", total)` does.
4. Lines within a stanza are short complete statements (each roughly 3-7 tokens). Do not merge two steps onto one line — renga is made of small linked verses.
5. When the chain includes an average step, use INTEGER division `//` exactly as the example does — `avg = total // count`. Float division (`/`) prints `5.714...` and fails the output check; the reference reports the floor average.
6. Replace the example's data handling with the real task work; never leave poetic filler, dead assignments, or fake output. Each stanza must carry real work for its stage.
7. After every edit, run the program for the requested input, then run `scripts/rhythm_check.py solve.py`. If it reports the wrong alternation, add or split a real step to restore 3-2-3.

#!/usr/bin/env python3
"""Rhythm checker for the haiku skill.

Prints the token profile of a code haiku and fails any line outside +/-2 of the
5-7-5 silhouette. The budget is conserved when fewer lines are used: three
lines ~5/7/5, two lines ~12/5 (setup+turn, then the landing), one line ~17.
Rhythm is not optional at any line count, and the landing line stays the short
~5. Mirrors the benchmark grader's counting convention exactly (whitespace
tokens; imports, comments, and docstring openers are free ceremony), so a pass
here means a pass in the mechanical form check.

Usage:
    python3 scripts/rhythm_check.py solve.py          # check a file
    python3 scripts/rhythm_check.py                   # defaults to ./solve.py
    python3 scripts/rhythm_check.py solve.py --target 7,7,7,5   # other forms
"""
import argparse
import re
import sys
...
def main():
    ap = argparse.ArgumentParser(
        description="Print the 5-7-5 token silhouette of a code haiku and fail any line outside +/-2."
    )
